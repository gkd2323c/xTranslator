...
fn benchmark_memory_usage() {
    ...
    assert!(
        memory_per_string < 5000,
        "Per-string memory too high: {} bytes",
        memory_per_string
    );
}
...
fn benchmark_filtering() {
    ...
    let _regex_filtered: Vec<_> = parser
        .strings
        .iter()
    ...
    let start = Instant::now();
    let _record_filtered: Vec<_> = parser
        .strings
        .iter()
    ...
    let start = Instant::now();
    let _status_filtered: Vec<_> = parser
        .strings
        .iter()
    ...
    // Performance assertions
    assert!(
        text_filter_time < Duration::from_millis(100),
        "Text filtering too slow"
    );
    assert!(
        regex_filter_time < Duration::from_millis(200),
        "Regex filtering too slow"
    );
    assert!(
        record_filter_time < Duration::from_millis(50),
        "Record filtering too slow"
    );
    assert!(
        status_filter_time < Duration::from_millis(50),
        "Status filtering too slow"
    );
}
...
fn benchmark_sorting() {
    ...
    // Performance assertions
    assert!(
        id_sort_time < Duration::from_millis(500),
        "ID sorting too slow"
    );
    assert!(
        source_sort_time < Duration::from_secs(1),
        "Source sorting too slow"
    );
    assert!(
        record_sort_time < Duration::from_millis(500),
        "Record sorting too slow"
    );
}
...
fn benchmark_file_io() {
    ...
    // Performance assertions
    assert!(
        strings_save_time < Duration::from_secs(5),
        "Strings save too slow"
    );
    assert!(
        strings_load_time < Duration::from_secs(2),
        "Strings load too slow"
    );
...
fn benchmark_heuristic_search() {
    ...
    let translated_strings: Vec<_> = parser
        .strings
        .iter()
        .filter(|s| s.params.is_translated())
        .collect();
    ...
    for query in queries {
        let start = Instant::now();
        let _results: Vec<_> = translated_strings
            .iter()
            .filter(|s| {
    ...
    println!(
        "🔍 Heuristic Search Performance ({} translated strings):",
        translated_count
    );
}
...
fn benchmark_translation_api() {
    ...
    for (i, string) in test_strings.iter().enumerate() {
        ...
        println!(
            "🌐 Translation API - '{}': {:?} -> '{}'",
            string, total_time, translation
        );

        // API should respond within reasonable time
        assert!(
            total_time < Duration::from_millis(500),
            "Translation API too slow"
        );
    }
}
...
fn benchmark_memory_pressure() {
    ...
    for i in 0..5 {
        ...
        println!(
            "🧠 Memory pressure test - Parser {}: {} strings",
            i + 1,
            parser.strings.len()
        );
        parsers.push(parser);
    ...
    for (i, parser) in parsers.iter().enumerate() {
        ...
        println!(
            "  Parser {} - {} translated strings, filter time: {:?}",
            i + 1,
            count,
            filter_time
        );
        assert!(
            ...
            "Filter too slow under memory pressure"
        );
    }
}
...
fn benchmark_stress_test() {
    ...
    assert!(
        ...
        "Large dataset filtering too slow"
    );
    assert!(
        sort_time < Duration::from_secs(2),
        "Large dataset sorting too slow"
    );
    assert!(
        sst_time < Duration::from_secs(5),
        "Large SST creation too slow"
    );
}
