...
- [ ] 1.1 Create `src-tauri/src/batch.rs` with `BatchExecutor`, `BatchConfig`, `BatchEntry`, `BatchStatus`, `BatchProgress` types
- [ ] 1.2 Implement `BatchExecutor::new()` — empty state, cancel flag
- [ ] 1.3 Implement `BatchExecutor::validate_entries()` — check .esp exists, strings dir exists or fallback
- [ ] 1.4 Implement `BatchExecutor::start_translate()` — spawn tokio task, sequential file processing loop
- [ ] 1.5 Implement per-file flow: parse ESP → filter untranslated → translate each → save strings
- [ ] 1.6 Implement cancel: check `AtomicBool` between files and between API calls
- [ ] 1.7 Implement error handling: per-string skip, per-file skip, API key abort
- [ ] 1.8 Implement `start_export()` — sequential ESP load → export per file
- [ ] 1.9 Implement `get_status()` → `Option<BatchStatus>`
- [ ] 1.10 Wire up Tauri event emission: `batch-progress`, `batch-file-complete`, `batch-complete`

## 2. Backend — IPC Commands

- [ ] 2.1 Add `start_batch_translate` command — parse config, delegate to BatchExecutor
- [ ] 2.2 Add `start_batch_export` command — parse config, delegate to BatchExecutor
- [ ] 2.3 Add `get_batch_status` command — return current BatchStatus or null
- [ ] 2.4 Add `cancel_batch_job` command — set cancel flag
- [ ] 2.5 Register all 4 commands in `main.rs` + add `Arc<BatchExecutor>` to `manage()`
...
- [ ] 6.1 Unit tests for `BatchExecutor` in `src-tauri/src/batch.rs`
- [ ] 6.2 Integration test: 2-file batch with real ESP test fixtures
- [ ] 6.3 Test cancel flow with mock translation provider
- [ ] 6.4 Test error recovery: corrupted file, network failure simulation
...
- [ ] 6.8 Manual E2E: 3-file batch translate with DeepL on real Skyrim follower mods
