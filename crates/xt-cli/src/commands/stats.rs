...
pub fn dump_stats(input: &str, game: Option<GameId>) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
        ...
        if strings_dir.exists() {
            ...
            if loaded > 0 {
                println!("  Loaded {}/3 strings files from: {}", loaded, strings_dir.display());
            }
        }
    }
    ...
    println!("| 游戏 | {} |", game.map(|g| format!("{:?}", g)).unwrap_or_else(|| "Auto".to_string()));
    ...
    println!("| 不同 record:field 组合数 | {} |", record_field_counts.len());
    ...
    fn count_grup(grup: &xt_core::esp::record_tree::EspGrup, sub_count: &mut usize, rec_count: &mut usize) {
    ...
    let vmad_strings: Vec<_> = parser.strings.iter().filter(|s| (s.esp_ptr.str_id) < 0).collect();
    ...
    // ── 游戏定义覆盖 ───────────────────────────────────────────
    let defined_sigs: HashSet<String> = parser.record_defs.iter()
    ...
    println!("| 未覆盖的 record_sig | {} |", if uncovered.is_empty() { "无".to_string() } else { uncovered.join(", ") });
