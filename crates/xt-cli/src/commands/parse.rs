...
pub fn apply_sst(esp_path: &str, sst_path: &str, output: Option<&str>) -> Result<()> {
    ...
    println!("\nParsing done in {:.2}s, {} strings", elapsed.as_secs_f64(), parser.strings.len());
    ...
    let translated = parser.strings.iter()
        .filter(|s| s.params.is_translated())
        .count();
    let incomplete = parser.strings.iter()
        .filter(|s| s.params.is_incomplete())
        .count();

    println!("\nTranslation status:");
    println!("  Total: {}", total);
    println!("  Translated: {} ({:.1}%)", translated, 100.0 * translated as f64 / total as f64);
    println!("  Incomplete: {} ({:.1}%)", incomplete, 100.0 * incomplete as f64 / total as f64);
    ...
    println!("\nSample strings (first 20):");
    for sk in parser.strings.iter().take(20) {
        let status = if sk.params.is_translated() { "✓" } else if sk.params.is_incomplete() { "○" } else { "·" };
        println!(
            "  [{}] {}:{} | {} → {}",
            status,
            String::from_utf8_lossy(&sk.esp_ptr.record_sig),
