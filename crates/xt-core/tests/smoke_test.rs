...
fn smoke_xml_roundtrip() {
    ...
    assert!(exported_count >= 3, "Too few entries exported: {}", exported_count);
    ...
    for (i, (_, translation)) in test_translations.iter().enumerate() {
        if i < import_strings.len() {
            assert!(import_strings[i].translation.contains(translation), 
...
fn smoke_performance_validation() {
    ...
    assert!(string_count > 70000, "Too few strings loaded: {}", string_count);
}
...
fn smoke_error_handling() {
    ...
    assert!(empty_sst.save_to_file(tmp.to_str().unwrap()).is_ok(), 
...
fn smoke_data_integrity() {
    ...
    for (i, s) in parser.strings.iter().take(100).enumerate() {
        ...
        assert!(!s.record_sig.is_empty(), "String {} has empty record sig", i);
    ...
    assert!(record_counts.len() > 5, "Too few record types: {:?}", record_counts.keys());
    ...
    println!("📊 Compression ratio: {:.2}% ({}/{})", 
             compression_ratio * 100.0, compressed_count, total_count);
...
fn smoke_multi_format_validation() {
    ...
    for s in &parser.strings {
        if s.params.is_translated() && !s.translation.is_empty() {
            sf_null.strings.insert(s.esp_ptr.str_id as u32, s.translation.clone());
        }
    }
    sf_null.save(&null_path).unwrap();

    let reloaded_null = StringsFile::load_with_format(
        &null_path, 
    ...
    for s in &parser.strings {
        if s.params.is_translated() && !s.translation.is_empty() {
            sf_length.strings.insert(s.esp_ptr.str_id as u32, s.translation.clone());
        }
    }
    ...
    assert_eq!(reloaded_null.strings.len(), reloaded_length.strings.len(), 
               "Format mismatch in string count");

    for (id, translation) in &reloaded_null.strings {
        assert_eq!(reloaded_length.strings.get(id), Some(translation), 
                   "Translation mismatch for ID {} between formats", id);
    }
