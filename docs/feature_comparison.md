...
> **重写版本**：v1.2.0 — 后端 ~90%（359 单元测试），前端 ~80%（多游戏上下文已贯穿主流程）
>
...
| **MCM/Translate** | ✅ MCM 菜单翻译文件 | ✅ 后端解析+UI面板 | ~80% | 后端：parser+types+IPC命令；前端：McmPanel，含加载/保存/编辑/过滤/compare、side-by-side 对照、copy source、char ratio、Clear filtered |
...
| **翻译 API** | ✅ DeepL/MS/Google/OpenAI/Youdao/Baidu | ✅ OpenAI + DeepL + Baidu + Youdao + Azure + Google + API config | ~90% | 六个常用 provider 已接通；剩余工作主要是 Delphi 实机对照和真实服务验证，而非缺失实现 |
...
| **MCM Compare** | ✅ | ✅ reference file compare + overwrite policy | ~85% | `mcm_compare` IPC + `McmPanel` compare dialog，支持 matched/unmatched 统计、覆盖策略和对照编辑 |
...
| **配置系统** | ✅ res.ini+注册表 | ✅ JSON 配置持久化 + Proxy UI | ~95% | `AppConfig` JSON 持久化（theme/language/API key/proxy/spell check state），`load_config`/`save_config` IPC 命令，启动时自动加载；HTTP proxy 后端已接入 `build_client()`，SettingsDialog Proxy UI 已完成 |
...
| 功能 | 原版 | Rust 重写 | 覆盖度 | 说明 |
|------|------|----------|--------|------|
| **主窗口布局** | ✅ 菜单栏+文件树+编辑区 | ✅ 三阶段布局复刻 | ~80% | MenuBar + SidePanel + StringTable + StatusBar + 底部面板 + EditorDialog；工具弹窗面板 9 个，主表元数据列宽可调 |
| **虚拟字符串表格** | ✅ VirtualTreeView | ✅ react-window | ~85% | 76K+ 条虚拟滚动，多选(Ctrl/Shift+Click) + 键盘导航(PageUp/Down/Home/End) + 右键批量操作 + EDID/ID/LD 列宽拖拽 |
...
| **主题支持** | ✅ 默认/亮/灰/暗 | ✅ Obsidian/Slate/Light/Auto | ~90% | CSS variables + Zustand + localStorage, system follow via matchMedia |
...
| ~~翻译 API 集成~~ | ✅ OpenAI + DeepL + Baidu + Youdao + Azure + Google 已实现 | Done |
...
| ~~主题系统~~ | ✅ Obsidian/Slate/Light/Auto | Done |
...
| TESVT_TranslatorApi.pas | ✅ 已分析 | ✅ OpenAI + DeepL + Baidu + Youdao + Azure + Google + API config 解析 |
